...
<div class="max-w-7xl mx-auto sm:px-6">
    <form method="GET" action="{{ request()->url() }}"
          class="w-full bg-[#0d1b3d] border p-4 flex gap-3 items-end">
        ...
        <div class="flex gap-2 ml-auto">
            @if(request()->hasAny(['sort', 'score_min', 'date_from', 'date_to', 'proposer_id']))
                <a href="{{ request()->url() }}"
                   class="flex items-center gap-1 border border-slate-700 text-slate-400 rounded-lg px-4 py-2 text-sm hover:bg-slate-700 hover:text-slate-200 transition">
