...
?>
<div
    class="relative border border-white/25 bg-gray-50 text-sm font-medium hover:border-blue-500 transition-colors rounded-2xl p-3 sm:p-5 flex flex-col gap-3">

    <a href="{{ route('projects-details', $project) }}" class="absolute inset-0 z-0 text-gray-700"
    ...
    <div class="relative z-10 pointer-events-none flex flex-col items-start gap-2 py-2">
        ...
        <div class="flex flex-row justify-between w-full">
            ...
            <x-conditional-button :leader_id="$project->leader_id" :project="$project->id" />
        </div>
            ...
            @elseif($project->isInProgress())
                @if (
                    $project->progress >= 100 &&
                        auth()->user()?->can('complete project') &&
                        (auth()->id() === $project->leader_id || auth()->user()?->hasRole(\App\Enums\Role::ProjectManager->value)))
    ...
    @if ($project->isInRecolte() || $project->isInProgress())
        ...
        <div class="relative z-10 flex justify-end gap-2">
            ...
            @if ($project->isInRecolte() && auth()->user()?->can('launch project'))
                @php
                    $canLaunch = auth()->id() === $project->leader_id;
                ...
                <form action="{{ route('projects.recolte.activate', $project) }}" method="POST" class="relative z-10">
                    ...
                    <x-projects.buttons text="Démarrer le projet"
                        ...
                        type="submit" :disabled="!$canLaunch && !$isAdmin" />
                </form>
            @endif
        </div>
    @endif
    <div class="relative z-10 pointer-events-none flex items-center justify-between mt-1">
        ...
        @if ($updatedAt instanceof Carbon)
            ...
            ?>
            <span
                class="inline-flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-full {{ $bgColor }} {{ $textColor }} ring-1 ring-inset ring-black/5">
