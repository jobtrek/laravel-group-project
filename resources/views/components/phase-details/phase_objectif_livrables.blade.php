...
<div
    ...
    <div class="flex flex-col gap-4">
        @if($livrables_text)
            <p class="text-sm font-semibold" :class="checked ? 'text-green-800' : 'text-gray-800'">{{ $livrables_text }}</p>
        @elseif($objectifs_text)
            <p class="text-sm font-semibold" :class="checked ? 'text-green-800' : 'text-gray-800'">{{ $objectifs_text }}</p>
            @endif
        </div>
        <input
            x-model="checked"
