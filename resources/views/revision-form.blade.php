<x-app-layout>
    ...
    <div class="py-10">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex items-start gap-3 rounded-lg bg-amber-50 border border-amber-200 p-4 text-sm text-amber-800">
                ...
                @if ($comments->hasAny(['title', 'description', 'but', 'perimetre']))
                    ...
                    @if ($comments->has('but'))
                        <x-revision.flagged-field label="Objectifs SMART" :comment="$comments['but']->content">
                            <div x-data="{ items: @js($old['but'] ?? $project->but) }" class="mt-1 space-y-2">
                                <template x-for="(item, idx) in items" :key="idx">
                                    <div class="flex gap-2">
                                        <input
                                            type="text"
                                            :name="'corrections[but][' + idx + ']'"
                                            x-model="items[idx]"
                                            required
                    ...
                    @if ($comments->has('perimetre'))
                        <x-revision.flagged-field label="Périmètre" :comment="$comments['perimetre']->content">
                            <textarea
                                name="corrections[perimetre]"
                                rows="4"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                                       focus:ring-indigo-500 focus:border-indigo-500 resize-none"
                            >{{ $old['perimetre'] ?? $project->perimetre }}</textarea>
                        </x-revision.flagged-field>
                    @endif
                @endif
                ...
                @if ($comments->hasAny(['evaluation.portee', 'evaluation.impact', 'evaluation.confiance', 'evaluation.effort']))
                    ...
                    @if ($comments->has('evaluation.portee'))
                        <x-revision.flagged-field label="Portée (0 – 50)" :comment="$comments['evaluation.portee']->content">
                            <input
                                type="number"
                                name="corrections[evaluation.portee]"
                                value="{{ $old['evaluation.portee'] ?? $project->evaluation->portee }}"
                    ...
                    @if ($comments->has('evaluation.impact'))
                        ...
                        <x-revision.flagged-field label="Impact (1 – 5)" :comment="$comments['evaluation.impact']->content">
                            <select
                                name="corrections[evaluation.impact]"
                                required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                                       focus:ring-indigo-500 focus:border-indigo-500"
                            >
                                ...
                                <option value="3" @selected($currentImpact == 3)>3 — Crée une nouvelle capacité pour Jobtrek</option>
                                <option value="4" @selected($currentImpact == 4)>4 — Change considérablement le fonctionnement</option>
                    ...
                    @if ($comments->has('evaluation.confiance'))
                        <x-revision.flagged-field label="Confiance (0 – 100 %)" :comment="$comments['evaluation.confiance']->content">
                            <div class="flex items-center gap-2 mt-1">
                                <input
                                    type="number"
                                    name="corrections[evaluation.confiance]"
                                    value="{{ $old['evaluation.confiance'] ?? $project->evaluation->confiance }}"
                    ...
                    @if ($comments->has('evaluation.effort'))
                        ...
                        <x-revision.flagged-field label="Effort (1 – 5)" :comment="$comments['evaluation.effort']->content">
                            <select
                                name="corrections[evaluation.effort]"
                                required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                                       focus:ring-indigo-500 focus:border-indigo-500"
                            >
                                ...
                                <option value="2" @selected($currentEffort == 2)>2 — Quelques semaines (1 à 4 semaines ETP)</option>
                    ...
                    @if ($hasPhaseComments)
                        ...
                        @if ($comments->has("phases.{$i}.titre"))
                            <x-revision.flagged-field label="Titre" :comment="$comments['phases.' . $i . '.titre']->content">
                                <input
                                    type="text"
                                    name="corrections[phases.{{ $i }}.titre]"
                                    value="{{ $old["phases.{$i}.titre"] ?? $phase->name }}"
                                    required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                                           focus:ring-indigo-500 focus:border-indigo-500"
                                >
                        ...
                        @if ($comments->has("phases.{$i}.duree"))
                            <x-revision.flagged-field label="Durée" :comment="$comments['phases.' . $i . '.duree']->content">
                                <input
                                    type="text"
                                    name="corrections[phases.{{ $i }}.duree]"
                                    value="{{ $old["phases.{$i}.duree"] ?? $phase->duration }}"
                                    required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                                           focus:ring-indigo-500 focus:border-indigo-500"
                                >
                        ...
                        @if ($comments->has("phases.{$i}.description"))
                            <x-revision.flagged-field label="Description" :comment="$comments['phases.' . $i . '.description']->content">
                                <textarea
                                    name="corrections[phases.{{ $i }}.description]"
                                    rows="4"
                                    required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm text-sm
                                           focus:ring-indigo-500 focus:border-indigo-500 resize-none"
                                >{{ $old["phases.{$i}.description"] ?? $phase->description }}</textarea>
                            </x-revision.flagged-field>
                        @endif

                        @if ($comments->has("phases.{$i}.objectifs"))
                            <x-revision.flagged-field label="Objectifs" :comment="$comments['phases.' . $i . '.objectifs']->content">
                                <div x-data="{ items: @js($old["phases.{$i}.objectifs"] ?? $phase->objectifs) }" class="mt-1 space-y-2">
                                    <template x-for="(item, idx) in items" :key="idx">
                                        <div class="flex gap-2">
                                            <input
                                                type="text"
                                                :name="'corrections[phases.{{ $i }}.objectifs][' + idx + ']'"
                                                x-model="items[idx]"
                                                required
                                                class="block w-full rounded-md border-gray-300 shadow-sm text-sm
                                                       focus:ring-indigo-500 focus:border-indigo-500"
                        ...
                        @if ($comments->has("phases.{$i}.livrables"))
                            <x-revision.flagged-field label="Livrables" :comment="$comments['phases.' . $i . '.livrables']->content">
                                <div x-data="{ items: @js($old["phases.{$i}.livrables"] ?? $phase->livrables) }" class="mt-1 space-y-2">
                                    <template x-for="(item, idx) in items" :key="idx">
                                        <div class="flex gap-2">
                                            <input
                                                type="text"
                                                :name="'corrections[phases.{{ $i }}.livrables][' + idx + ']'"
                                                x-model="items[idx]"
                                                required
                                                class="block w-full rounded-md border-gray-300 shadow-sm text-sm
                                                       focus:ring-indigo-500 focus:border-indigo-500"
                                ...
                                <div x-data="{ resources: @js($initResources) }" class="mt-1 space-y-2">
                                    <template x-for="(res, ri) in resources" :key="ri">
                                        <div class="flex gap-2 items-center">
                                            ...
                                            <button
                                                type="button"
                                                @click="resources.splice(ri, 1)"
                                                x-show="resources.length > 1"
                                                class="px-2 py-1 text-red-500 hover:text-red-700 text-lg font-bold leading-none"
                                            >&times;</button>
                                        </div>
                                    </template>
                                    <button type="button"
                                        @click="resources.push({ resource_type: '', amount_needed: '' })"
                                        class="mt-1 text-sm text-indigo-600 hover:underline">
